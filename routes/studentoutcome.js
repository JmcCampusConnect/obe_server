...
// Check whether the staff is Tutor, Course Handler or HOD fir displaying buttons to show the Student Outcome
...
const getUniqueValues = (data, key) => { return [...new Set(data.map(entry => entry[key]))] }
...
// ------------------------------------------------------------------------------------------------------- //
// -------------------- COMMON STUDENT OUTCOME FUNCTION -------------------------------------------------- //
// ------------------------------------------------------------------------------------------------------- //
...
async function getStudentOutcome({ academicSem, semester, dept_id, category, section, staff_id }) {
    ...
    if (staff_id) {
        ...
        course_codes = course_mapping.map(m => m.course_code);
    ...
    if (staff_id && course_codes.length > 0) {
        markWhere.course_code = course_codes;
    }

    const marks = await markentry.findAll({ where: markWhere });

    // Fetch academic and calculation data
...
// ------------------------------------------------------------------------------------------------------- //
// -------------------- REPLACED ROUTES USING COMMON FUNCTION -------------------------------------------- //
// ------------------------------------------------------------------------------------------------------- //
